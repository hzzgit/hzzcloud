...
package templatestree.yrgpstranspond.service;
/**
                ...
                */
                public Map depNodeTree(Integer id, List<Department> depList) {
                    ...
                    List<Long> depids = new ArrayList<>();
                        String sql = "select * from subiaodb.yrgpstranspondbydep where mainid = ?  and deleted = 0";
                        List<Yrgpstranspondbydep> datas = jdbcUtil.sql(sql).addIndexParam(id).query(Yrgpstranspondbydep.class);
                        Map<Long, Boolean> autoischeckMap = new HashMap<>();
                        if (ConverterUtils.isList(datas)) {
                        for (Yrgpstranspondbydep gpstransferbydep : datas) {
                        ...
                        List<String> dellist = new ArrayList<>();
                            ...
                            * @param deletedepId
                            * @param adddepIds
                            * @param id 奕人gps转发基于protobuffer协议配置表id
                            */
                            @Transactional
                            ...
                            String sql = "update yrgpstranspondbydep set deleted = 1 where mainid = ? and depid = ?";
                            jdbcUtil.sql(sql).addIndexParam(id, s).executeUpdate();
                            }
                            }
                            if (adddepIds != null) {
                            for (String depId : adddepIds) {
                            Yrgpstranspondbydep yrgpstranspondbydep  = new Yrgpstranspondbydep ();
                            yrgpstranspondbydep.setMainid(id);
                            yrgpstranspondbydep.setCreatedate(new Date());
                            yrgpstranspondbydep.setDeleted(new Long(0));
                            yrgpstranspondbydep.setId(new Long(0));
                            yrgpstranspondbydep.setDepid(ConverterUtils.toLong(depId));
                            jdbcUtil.insert( yrgpstranspondbydep).execute();
                            }
                            }

                            } catch (Exception e) {
                            log.error("保存奕人gps转发基于protobuffer协议配置表机构授权配置出错", e);
                            throw e;
                            }
                            }
                            ...
                            * 保存配置的车辆授权表表
                            *
                            * @param id    奕人gps转发基于protobuffer协议配置表配置id,id
                            ...
                            */
                            ...
                            String sql = "update subiaodb.yrgpstranspondbyvehicle set deleted = 1 where mainid = ? and vehicleId = ?";
                            jdbcUtil.sql(sql).addIndexParam(id, deletevehicleId).executeUpdate();
                            }
                            }

                            List< Yrgpstranspondbyvehicle> yrgpstranspondbyvehicles = new ArrayList<>();
                            if (addvehicleIds != null && addvehicleIds.length > 0) {//这边是添加的车辆批量添加
                            for (String vehicleId : addvehicleIds) {
                            Yrgpstranspondbyvehicle yrgpstranspondbyvehicle=new Yrgpstranspondbyvehicle();
                            yrgpstranspondbyvehicle.setMainid(id);
                            yrgpstranspondbyvehicle.setCreatedate(new Date());
                            yrgpstranspondbyvehicle.setDeleted(0L);
                            yrgpstranspondbyvehicle.setVehicleid(ConverterUtils.toLong(vehicleId));
                            yrgpstranspondbyvehicle.setId(0L);
                            yrgpstranspondbyvehicles.add(yrgpstranspondbyvehicle);
                            }

                            LoopUtil.splitLoop(yrgpstranspondbyvehicles, 1000, list -> {
                            try {
                            jdbcUtil.insertList(list).setLog(log, "批量插入奕人gps转发基于protobuffer协议配置表车辆配置").setNotPrint()
                            .executeBatch(false);
                            } catch (Exception e) {
                            log.debug("插入奕人gps转发基于protobuffer协议配置表车辆配置报错，转为单条插入", e);
                            for (Yrgpstranspondbyvehicle yrgpstranspondbyvehicle : yrgpstranspondbyvehicles) {
                            try {
                            jdbcUtil.insert(yrgpstranspondbyvehicle).execute();
                            } catch (Exception e1) {
                            log.error("单条插入奕人gps转发基于protobuffer协议配置表车辆报错", e);
                            }
                            }
                            }
                            });
                            ...
                            log.error("保存奕人gps转发基于protobuffer协议配置表车辆授权配置出错", e);
                            throw e;
                            }
                            return arg;
                            }
                            ...
                            */
                            ...
                            String sql = "select * from yrgpstranspondbyvehicle where mainId=?  and deleted = 0";
                            List<Yrgpstranspondbyvehicle> yrgpstranspondbyvehicleList = jdbcUtil.sql(sql).addIndexParam(id).query(Yrgpstranspondbyvehicle.class);
                                        ...
                                        List<Long> vehicleIdList = uservehicleauthority.getVehicleIdList();
                                            ...
                                            List<Long> vedepList = new ArrayList<>();
                                                ...
                                                List<Department> departmentList = getAuthorizedDepList(depList);
                                                    ...
                                                    for (Yrgpstranspondbyvehicle yrgpstranspondbyvehicle : yrgpstranspondbyvehicleList) {
                                                    Long depId = new Long(0);
                                                    if (realDataServiceRedis.getVehiclemap().get(ConverterUtils.toString(yrgpstranspondbyvehicle.getVehicleid(), "")) != null) {
                                                    depId = realDataServiceRedis.getVehiclemap().get(ConverterUtils.toString(yrgpstranspondbyvehicle.getVehicleid(), "")).getDepId();
                                                    }
                                                    if (depMap.containsKey(depId) || vehicleMap.containsKey(yrgpstranspondbyvehicle.getVehicleid())) {
                                                    vehicleIds.add(yrgpstranspondbyvehicle.getVehicleid());
                                                    }
                                                    ...
                                                    Map<Long, List<HashMap>> vehiclesbydep = new HashMap<>();
                                                        ...
                                                        List<String> dellist = new ArrayList<>();
                                                            ...
                                                            List<ConfigTreeNode> childnodes = v.getChildren();
                                                                ...
                                                                String delsql = "delete from yrgpstransponduserlim where mainid=? and userid = ?";
                                                                jdbcUtil.sql(delsql).addIndexParam(id, deleteuserid).executeUpdate();
                                                                }
                                                                }

                                                                if (adduserIdsArr != null && adduserIdsArr.length > 0) {
                                                                List<Yrgpstransponduserlim> yrgpstransponduserlims = new ArrayList<>();
                                                                for (String userid : adduserIdsArr) {
                                                                Yrgpstransponduserlim yrgpstransponduserlim = new Yrgpstransponduserlim();
                                                                yrgpstransponduserlim.setMainid(id);
                                                                yrgpstransponduserlim.setUserid(Long.parseLong(userid));
                                                                yrgpstransponduserlims.add(yrgpstransponduserlim);
                                                                }
                                                                LoopUtil.splitLoop(yrgpstransponduserlims, 3000, list -> {
                                                                try {
                                                                ...
                                                                log.error("奕人gps转发基于protobuffer协议配置表用户授权配置保存出错", e);
                                                                throw e;
                                                                }
                                                                }
                                                                ...
                                                                List<ConfigTreeNode> vehicleTreeNodes = vehicleTreeNode.getChildren();
                                                                    ...
                                                                    */
                                                                    ...
                                                                    jdbcUtil.update(yrgpstranspond).whereIdRefValueEQ().updateColumn(ColumnSet.all().minus(Yrgpstranspond.F_createdate,
                                                                    Yrgpstranspond.F_deleted,Yrgpstranspond.F_id,Yrgpstranspond.F_userid)).execute();
                                                                    }
                                                                    }
