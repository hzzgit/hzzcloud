...
public class TakingphotosbytimeService {
    ...
    public Map depNodeTree(Integer id, List<Department> depList) {
        ...
        String sql = "select * from subiaodb.takingphotosbytimebydep where mainid = ?  and deleted = 0";
        List<Takingphotosbytimebydep> datas = jdbcUtil.sql(sql).addIndexParam(id).query(Takingphotosbytimebydep.class);
        Map<Long, Boolean> autoischeckMap = new HashMap<>();
        if (ConverterUtils.isList(datas)) {
            for (Takingphotosbytimebydep gpstransferbydep : datas) {
        ...
        try {
            if (deletedepId != null) {
                for (String s : deletedepId) {
                    String sql = "update takingphotosbytimebydep set deleted = 1 where mainid = ? and depid = ?";
                    jdbcUtil.sql(sql).addIndexParam(id, s).executeUpdate();
                }
            }
            if (adddepIds != null) {
                for (String depId : adddepIds) {
                    Takingphotosbytimebydep takingphotosbytimebydep = new Takingphotosbytimebydep();
                    takingphotosbytimebydep.setMainid(id);
                    takingphotosbytimebydep.setCreatedate(new Date());
                    takingphotosbytimebydep.setDeleted(new Long(0));
                    takingphotosbytimebydep.setId(new Long(0));
                    takingphotosbytimebydep.setDepid(ConverterUtils.toLong(depId));
                    jdbcUtil.insert(takingphotosbytimebydep).execute();
                }
            }

        } catch (Exception e) {
            log.error("保存定时拍照配置表机构授权配置出错", e);
            throw e;
        }
    }
        ...
        try {
            if (deletevehicleIds != null && deletevehicleIds.length > 0) {//这边是删除车辆
                for (String deletevehicleId : deletevehicleIds) {
                    String sql = "update subiaodb.takingphotosbytimebyvehicle set deleted = 1 where mainid = ? and vehicleId = ?";
                    jdbcUtil.sql(sql).addIndexParam(id, deletevehicleId).executeUpdate();
                }
            }

            List<Takingphotosbytimebyvehicle> takingphotosbytimebyvehicles = new ArrayList<>();
            if (addvehicleIds != null && addvehicleIds.length > 0) {//这边是添加的车辆批量添加
                for (String vehicleId : addvehicleIds) {
                    Takingphotosbytimebyvehicle takingphotosbytimebyvehicle = new Takingphotosbytimebyvehicle();
                    takingphotosbytimebyvehicle.setMainid(id);
                    takingphotosbytimebyvehicle.setCreatedate(new Date());
                    takingphotosbytimebyvehicle.setDeleted(0L);
                    takingphotosbytimebyvehicle.setVehicleid(ConverterUtils.toLong(vehicleId));
                    takingphotosbytimebyvehicle.setId(0L);
                    takingphotosbytimebyvehicles.add(takingphotosbytimebyvehicle);
                }

                LoopUtil.splitLoop(takingphotosbytimebyvehicles, 1000, list -> {
                    try {
                        jdbcUtil.insertList(list).setLog(log, "批量插入定时拍照配置表车辆配置").setNotPrint()
                                .executeBatch(false);
                    } catch (Exception e) {
                        log.debug("插入定时拍照配置表车辆配置报错，转为单条插入", e);
                        for (Takingphotosbytimebyvehicle takingphotosbytimebyvehicle : takingphotosbytimebyvehicles) {
                            try {
                                jdbcUtil.insert(takingphotosbytimebyvehicle).execute();
                            } catch (Exception e1) {
                                log.error("单条插入定时拍照配置表车辆报错", e);
                            }
                        }
                    }
                });
    ...
    public Map depandceNodeTree(Integer id, Uservehicleauthority uservehicleauthority) {
        String sql = "select * from takingphotosbytimebyvehicle where mainId=?  and deleted = 0";
        List<Takingphotosbytimebyvehicle> takingphotosbytimebyvehicleList = jdbcUtil.sql(sql).addIndexParam(id).query(Takingphotosbytimebyvehicle.class);
        ...
        for (Takingphotosbytimebyvehicle takingphotosbytimebyvehicle : takingphotosbytimebyvehicleList) {
            Long depId = new Long(0);
            if (realDataServiceRedis.getVehiclemap().get(ConverterUtils.toString(takingphotosbytimebyvehicle.getVehicleid(), "")) != null) {
                depId = realDataServiceRedis.getVehiclemap().get(ConverterUtils.toString(takingphotosbytimebyvehicle.getVehicleid(), "")).getDepId();
            }
            if (depMap.containsKey(depId) || vehicleMap.containsKey(takingphotosbytimebyvehicle.getVehicleid())) {
                vehicleIds.add(takingphotosbytimebyvehicle.getVehicleid());
            }
        ...
        try {
            if (deleteuserIdsArr != null && deleteuserIdsArr.length > 0) {
                for (String deleteuserid : deleteuserIdsArr) {
                    String delsql = "delete from takingphotosbytimeuserlim where mainid=? and userid = ?";
                    jdbcUtil.sql(delsql).addIndexParam(id, deleteuserid).executeUpdate();
                }
            }

            if (adduserIdsArr != null && adduserIdsArr.length > 0) {
                List<Takingphotosbytimeuserlim> takingphotosbytimeuserlims = new ArrayList<>();
                for (String userid : adduserIdsArr) {
                    Takingphotosbytimeuserlim takingphotosbytimeuserlim = new Takingphotosbytimeuserlim();
                    takingphotosbytimeuserlim.setMainid(id);
                    takingphotosbytimeuserlim.setUserid(Long.parseLong(userid));
                    takingphotosbytimeuserlims.add(takingphotosbytimeuserlim);
                }
                LoopUtil.splitLoop(takingphotosbytimeuserlims, 3000, list -> {
                    try {
     ...
     * 保存定时拍照配置表
     */
    public void save(Takingphotosbytime takingphotosbytime, Long userid) throws Exception {
        if (takingphotosbytime.getId() == 0) {//新增
            takingphotosbytime.setCreatedate(new Date());
            takingphotosbytime.setUserid(userid);
            jdbcUtil.insert(takingphotosbytime).insertColumn(ColumnSet.all()).execute();
        } else {//修改
            takingphotosbytime.setUpdatedate(new Date());
            jdbcUtil.update(takingphotosbytime).whereIdRefValueEQ().updateColumn(ColumnSet.all().minus(Takingphotosbytime.F_createdate,
                    Takingphotosbytime.F_deleted, Takingphotosbytime.F_id, Takingphotosbytime.F_userid)).execute();
        }
    }
