package templatestree.flowlimitconfig.service;
/**
* 流量阈值配置表业务层
*/
                ...
                */
                public Map depNodeTree(Integer id, List<Department> depList) {
                    ...
                    List<Long> depids = new ArrayList<>();
                        String sql = "select * from subiaodb.flowlimitconfigbydep where mainid = ?  and deleted = 0";
                        List<Flowlimitconfigbydep> datas = jdbcUtil.sql(sql).addIndexParam(id).query(Flowlimitconfigbydep.class);
                        Map<Long, Boolean> autoischeckMap = new HashMap<>();
                        if (ConverterUtils.isList(datas)) {
                        for (Flowlimitconfigbydep gpstransferbydep : datas) {
                        ...
                        List<String> dellist = new ArrayList<>();
                            ...
                            */
                            ...
                            String sql = "update flowlimitconfigbydep set deleted = 1 where mainid = ? and depid = ?";
                            jdbcUtil.sql(sql).addIndexParam(id, s).executeUpdate();
                            }
                            }
                            if (adddepIds != null) {
                            for (String depId : adddepIds) {
                            Flowlimitconfigbydep flowlimitconfigbydep  = new Flowlimitconfigbydep ();
                            flowlimitconfigbydep.setMainid(id);
                            flowlimitconfigbydep.setCreatedate(new Date());
                            flowlimitconfigbydep.setDeleted(new Long(0));
                            flowlimitconfigbydep.setId(new Long(0));
                            flowlimitconfigbydep.setDepid(ConverterUtils.toLong(depId));
                            jdbcUtil.insert( flowlimitconfigbydep).execute();
                            }
                            }

                            } catch (Exception e) {
                            log.error("保存流量阈值配置表机构授权配置出错", e);
                            throw e;
                            }
                            }
                            ...
                            */
                            ...
                            String sql = "update subiaodb.flowlimitconfigbyvehicle set deleted = 1 where mainid = ? and vehicleId = ?";
                            jdbcUtil.sql(sql).addIndexParam(id, deletevehicleId).executeUpdate();
                            }
                            }

                            List< Flowlimitconfigbyvehicle> flowlimitconfigbyvehicles = new ArrayList<>();
                            if (addvehicleIds != null && addvehicleIds.length > 0) {//这边是添加的车辆批量添加
                            for (String vehicleId : addvehicleIds) {
                            Flowlimitconfigbyvehicle flowlimitconfigbyvehicle=new Flowlimitconfigbyvehicle();
                            flowlimitconfigbyvehicle.setMainid(id);
                            flowlimitconfigbyvehicle.setCreatedate(new Date());
                            flowlimitconfigbyvehicle.setDeleted(0L);
                            flowlimitconfigbyvehicle.setVehicleid(ConverterUtils.toLong(vehicleId));
                            flowlimitconfigbyvehicle.setId(0L);
                            flowlimitconfigbyvehicles.add(flowlimitconfigbyvehicle);
                            }

                            LoopUtil.splitLoop(flowlimitconfigbyvehicles, 1000, list -> {
                            try {
                            jdbcUtil.insertList(list).setLog(log, "批量插入流量阈值配置表车辆配置").setNotPrint()
                            .executeBatch(false);
                            } catch (Exception e) {
                            log.debug("插入流量阈值配置表车辆配置报错，转为单条插入", e);
                            for (Flowlimitconfigbyvehicle flowlimitconfigbyvehicle : flowlimitconfigbyvehicles) {
                            try {
                            jdbcUtil.insert(flowlimitconfigbyvehicle).execute();
                            } catch (Exception e1) {
                            log.error("单条插入流量阈值配置表车辆报错", e);
                            }
                            }
                            }
                            });
                            ...
                            */
                            ...
                            String sql = "select * from flowlimitconfigbyvehicle where mainId=?  and deleted = 0";
                            List<Flowlimitconfigbyvehicle> flowlimitconfigbyvehicleList = jdbcUtil.sql(sql).addIndexParam(id).query(Flowlimitconfigbyvehicle.class);
                                        ...
                                        List<Long> vehicleIdList = uservehicleauthority.getVehicleIdList();
                                            ...
                                            List<Long> vedepList = new ArrayList<>();
                                                ...
                                                List<Department> departmentList = getAuthorizedDepList(depList);
                                                    ...
                                                    for (Flowlimitconfigbyvehicle flowlimitconfigbyvehicle : flowlimitconfigbyvehicleList) {
                                                    Long depId = new Long(0);
                                                    if (realDataServiceRedis.getVehiclemap().get(ConverterUtils.toString(flowlimitconfigbyvehicle.getVehicleid(), "")) != null) {
                                                    depId = realDataServiceRedis.getVehiclemap().get(ConverterUtils.toString(flowlimitconfigbyvehicle.getVehicleid(), "")).getDepId();
                                                    }
                                                    if (depMap.containsKey(depId) || vehicleMap.containsKey(flowlimitconfigbyvehicle.getVehicleid())) {
                                                    vehicleIds.add(flowlimitconfigbyvehicle.getVehicleid());
                                                    }
                                                    ...
                                                    Map<Long, List<HashMap>> vehiclesbydep = new HashMap<>();
                                                        ...
                                                        List<String> dellist = new ArrayList<>();
                                                            ...
                                                            List<ConfigTreeNode> childnodes = v.getChildren();
                                                                ...
                                                                String delsql = "delete from flowlimitconfiguserlim where mainid=? and userid = ?";
                                                                jdbcUtil.sql(delsql).addIndexParam(id, deleteuserid).executeUpdate();
                                                                }
                                                                }

                                                                if (adduserIdsArr != null && adduserIdsArr.length > 0) {
                                                                List<Flowlimitconfiguserlim> flowlimitconfiguserlims = new ArrayList<>();
                                                                for (String userid : adduserIdsArr) {
                                                                Flowlimitconfiguserlim flowlimitconfiguserlim = new Flowlimitconfiguserlim();
                                                                flowlimitconfiguserlim.setMainid(id);
                                                                flowlimitconfiguserlim.setUserid(Long.parseLong(userid));
                                                                flowlimitconfiguserlims.add(flowlimitconfiguserlim);
                                                                }
                                                                LoopUtil.splitLoop(flowlimitconfiguserlims, 3000, list -> {
                                                                try {
                                                                ...
                                                                List<ConfigTreeNode> vehicleTreeNodes = vehicleTreeNode.getChildren();
                                                                    ...
                                                                    * 保存流量阈值配置表
                                                                    */
                                                                    public void save(Flowlimitconfig flowlimitconfig,Long userid) throws Exception {
                                                                    if (flowlimitconfig.getId() == 0) {//新增
                                                                    flowlimitconfig.setCreatedate(new Date());
                                                                    flowlimitconfig.setUserid(userid);
                                                                    jdbcUtil.insert(flowlimitconfig).insertColumn(ColumnSet.all()).execute();
                                                                    } else {//修改
                                                                    flowlimitconfig.setUpdatedate(new Date());
                                                                    jdbcUtil.update(flowlimitconfig).whereIdRefValueEQ().updateColumn(ColumnSet.all().minus(Flowlimitconfig.F_createdate,
                                                                    Flowlimitconfig.F_deleted,Flowlimitconfig.F_id,Flowlimitconfig.F_userid)).execute();
                                                                    }
                                                                    }
