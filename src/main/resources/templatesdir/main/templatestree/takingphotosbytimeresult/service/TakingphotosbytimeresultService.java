...
package templatestree.takingphotosbytimeresult.service;
/**
* 定时拍照结果表(总表)业务层
*/
                ...
                */
                public Map depNodeTree(Integer id, List<Department> depList) {
                    ...
                    List<Long> depids = new ArrayList<>();
                        String sql = "select * from subiaodb.takingphotosbytimeresultbydep where mainid = ?  and deleted = 0";
                        List<Takingphotosbytimeresultbydep> datas = jdbcUtil.sql(sql).addIndexParam(id).query(Takingphotosbytimeresultbydep.class);
                        Map<Long, Boolean> autoischeckMap = new HashMap<>();
                        if (ConverterUtils.isList(datas)) {
                        for (Takingphotosbytimeresultbydep gpstransferbydep : datas) {
                        ...
                        List<String> dellist = new ArrayList<>();
                            ...
                            */
                            ...
                            String sql = "update takingphotosbytimeresultbydep set deleted = 1 where mainid = ? and depid = ?";
                            jdbcUtil.sql(sql).addIndexParam(id, s).executeUpdate();
                            }
                            }
                            if (adddepIds != null) {
                            for (String depId : adddepIds) {
                            Takingphotosbytimeresultbydep takingphotosbytimeresultbydep  = new Takingphotosbytimeresultbydep ();
                            takingphotosbytimeresultbydep.setMainid(id);
                            takingphotosbytimeresultbydep.setCreatedate(new Date());
                            takingphotosbytimeresultbydep.setDeleted(new Long(0));
                            takingphotosbytimeresultbydep.setId(new Long(0));
                            takingphotosbytimeresultbydep.setDepid(ConverterUtils.toLong(depId));
                            jdbcUtil.insert( takingphotosbytimeresultbydep).execute();
                            }
                            }

                            } catch (Exception e) {
                            log.error("保存定时拍照结果表(总表)机构授权配置出错", e);
                            throw e;
                            }
                            }
                            ...
                            */
                            ...
                            String sql = "update subiaodb.takingphotosbytimeresultbyvehicle set deleted = 1 where mainid = ? and vehicleId = ?";
                            jdbcUtil.sql(sql).addIndexParam(id, deletevehicleId).executeUpdate();
                            }
                            }

                            List< Takingphotosbytimeresultbyvehicle> takingphotosbytimeresultbyvehicles = new ArrayList<>();
                            if (addvehicleIds != null && addvehicleIds.length > 0) {//这边是添加的车辆批量添加
                            for (String vehicleId : addvehicleIds) {
                            Takingphotosbytimeresultbyvehicle takingphotosbytimeresultbyvehicle=new Takingphotosbytimeresultbyvehicle();
                            takingphotosbytimeresultbyvehicle.setMainid(id);
                            takingphotosbytimeresultbyvehicle.setCreatedate(new Date());
                            takingphotosbytimeresultbyvehicle.setDeleted(0L);
                            takingphotosbytimeresultbyvehicle.setVehicleid(ConverterUtils.toLong(vehicleId));
                            takingphotosbytimeresultbyvehicle.setId(0L);
                            takingphotosbytimeresultbyvehicles.add(takingphotosbytimeresultbyvehicle);
                            }

                            LoopUtil.splitLoop(takingphotosbytimeresultbyvehicles, 1000, list -> {
                            try {
                            jdbcUtil.insertList(list).setLog(log, "批量插入定时拍照结果表(总表)车辆配置").setNotPrint()
                            .executeBatch(false);
                            } catch (Exception e) {
                            log.debug("插入定时拍照结果表(总表)车辆配置报错，转为单条插入", e);
                            for (Takingphotosbytimeresultbyvehicle takingphotosbytimeresultbyvehicle : takingphotosbytimeresultbyvehicles) {
                            try {
                            jdbcUtil.insert(takingphotosbytimeresultbyvehicle).execute();
                            } catch (Exception e1) {
                            log.error("单条插入定时拍照结果表(总表)车辆报错", e);
                            }
                            }
                            }
                            });
                            ...
                            log.error("保存定时拍照结果表(总表)车辆授权配置出错", e);
                            throw e;
                            }
                            return arg;
                            }
                            ...
                            */
                            ...
                            String sql = "select * from takingphotosbytimeresultbyvehicle where mainId=?  and deleted = 0";
                            List<Takingphotosbytimeresultbyvehicle> takingphotosbytimeresultbyvehicleList = jdbcUtil.sql(sql).addIndexParam(id).query(Takingphotosbytimeresultbyvehicle.class);
                                        ...
                                        List<Long> vehicleIdList = uservehicleauthority.getVehicleIdList();
                                            ...
                                            List<Long> vedepList = new ArrayList<>();
                                                ...
                                                List<Department> departmentList = getAuthorizedDepList(depList);
                                                    ...
                                                    for (Takingphotosbytimeresultbyvehicle takingphotosbytimeresultbyvehicle : takingphotosbytimeresultbyvehicleList) {
                                                    Long depId = new Long(0);
                                                    if (realDataServiceRedis.getVehiclemap().get(ConverterUtils.toString(takingphotosbytimeresultbyvehicle.getVehicleid(), "")) != null) {
                                                    depId = realDataServiceRedis.getVehiclemap().get(ConverterUtils.toString(takingphotosbytimeresultbyvehicle.getVehicleid(), "")).getDepId();
                                                    }
                                                    if (depMap.containsKey(depId) || vehicleMap.containsKey(takingphotosbytimeresultbyvehicle.getVehicleid())) {
                                                    vehicleIds.add(takingphotosbytimeresultbyvehicle.getVehicleid());
                                                    }
                                                    ...
                                                    Map<Long, List<HashMap>> vehiclesbydep = new HashMap<>();
                                                        ...
                                                        List<String> dellist = new ArrayList<>();
                                                            ...
                                                            List<ConfigTreeNode> childnodes = v.getChildren();
                                                                ...
                                                                String delsql = "delete from takingphotosbytimeresultuserlim where mainid=? and userid = ?";
                                                                jdbcUtil.sql(delsql).addIndexParam(id, deleteuserid).executeUpdate();
                                                                }
                                                                }

                                                                if (adduserIdsArr != null && adduserIdsArr.length > 0) {
                                                                List<Takingphotosbytimeresultuserlim> takingphotosbytimeresultuserlims = new ArrayList<>();
                                                                for (String userid : adduserIdsArr) {
                                                                Takingphotosbytimeresultuserlim takingphotosbytimeresultuserlim = new Takingphotosbytimeresultuserlim();
                                                                takingphotosbytimeresultuserlim.setMainid(id);
                                                                takingphotosbytimeresultuserlim.setUserid(Long.parseLong(userid));
                                                                takingphotosbytimeresultuserlims.add(takingphotosbytimeresultuserlim);
                                                                }
                                                                LoopUtil.splitLoop(takingphotosbytimeresultuserlims, 3000, list -> {
                                                                try {
                                                                ...
                                                                log.error("定时拍照结果表(总表)用户授权配置保存出错", e);
                                                                throw e;
                                                                }
                                                                }
                                                                ...
                                                                List<ConfigTreeNode> vehicleTreeNodes = vehicleTreeNode.getChildren();
                                                                    ...
                                                                    * 保存定时拍照结果表(总表)
                                                                    */
                                                                    public void save(Takingphotosbytimeresult takingphotosbytimeresult,long userid) throws Exception {
                                                                    ...
                                                                    jdbcUtil.update(takingphotosbytimeresult).whereIdRefValueEQ().updateColumn(ColumnSet.all().minus(Takingphotosbytimeresult.F_createdate,
                                                                    Takingphotosbytimeresult.F_deleted,Takingphotosbytimeresult.F_id,Takingphotosbytimeresult.F_userid)).execute();
                                                                    }
                                                                    }
