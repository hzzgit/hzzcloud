                    ...
                    List<Long> depids = new ArrayList<>();
                        ...
                        List<String> dellist = new ArrayList<>();
                            ...
                            */
                            ...
                            try {
                            jdbcUtil.insertList(list).setLog(log, "批量插入车辆配置").setNotPrint()
                            .executeBatch(false);
                            } catch (Exception e) {
                            log.debug("插入车辆配置报错，转为单条插入", e);
                            for (Vehiclebyvehicle vehiclebyvehicle : vehiclebyvehicles) {
                            try {
                            jdbcUtil.insert(vehiclebyvehicle).execute();
                            } catch (Exception e1) {
                            log.error("单条插入车辆报错", e);
                            }
                            }
                            }
                            });
