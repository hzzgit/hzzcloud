package templatestree.t_contract_park.service;
/**
* 车场合同业务层
*/
                ...
                */
                public Map depNodeTree(Integer id, List<Department> depList) {
                    ...
                    List<Long> depids = new ArrayList<>();
                        String sql = "select * from sync.t_contract_parkbydep where mainid = ?  and deleted = 0";
                        List<ContractParkbydep> datas = jdbcUtil.sql(sql).addIndexParam(id).query(ContractParkbydep.class);
                        Map<Long, Boolean> autoischeckMap = new HashMap<>();
                        if (ConverterUtils.isList(datas)) {
                        for (ContractParkbydep gpstransferbydep : datas) {
                        ...
                        List<String> dellist = new ArrayList<>();
                            ...
                            */
                            ...
                            String sql = "update t_contract_parkbydep set deleted = 1 where mainid = ? and depid = ?";
                            jdbcUtil.sql(sql).addIndexParam(id, s).executeUpdate();
                            }
                            }
                            if (adddepIds != null) {
                            for (String depId : adddepIds) {
                            ContractParkbydep t_contract_parkbydep  = new ContractParkbydep ();
                            t_contract_parkbydep.setMainid(id);
                            t_contract_parkbydep.setCreatedate(new Date());
                            t_contract_parkbydep.setDeleted(new Long(0));
                            t_contract_parkbydep.setId(new Long(0));
                            t_contract_parkbydep.setDepid(ConverterUtils.toLong(depId));
                            jdbcUtil.insert( t_contract_parkbydep).execute();
                            }
                            }

                            } catch (Exception e) {
                            log.error("保存车场合同机构授权配置出错", e);
                            throw e;
                            }
                            }
                            ...
                            */
                            ...
                            String sql = "update sync.t_contract_parkbyvehicle set deleted = 1 where mainid = ? and vehicleId = ?";
                            jdbcUtil.sql(sql).addIndexParam(id, deletevehicleId).executeUpdate();
                            }
                            }

                            List< ContractParkbyvehicle> t_contract_parkbyvehicles = new ArrayList<>();
                            if (addvehicleIds != null && addvehicleIds.length > 0) {//这边是添加的车辆批量添加
                            for (String vehicleId : addvehicleIds) {
                            ContractParkbyvehicle t_contract_parkbyvehicle=new ContractParkbyvehicle();
                            t_contract_parkbyvehicle.setMainid(id);
                            t_contract_parkbyvehicle.setCreatedate(new Date());
                            t_contract_parkbyvehicle.setDeleted(0L);
                            t_contract_parkbyvehicle.setVehicleid(ConverterUtils.toLong(vehicleId));
                            t_contract_parkbyvehicle.setId(0L);
                            t_contract_parkbyvehicles.add(t_contract_parkbyvehicle);
                            }

                            LoopUtil.splitLoop(t_contract_parkbyvehicles, 1000, list -> {
                            try {
                            jdbcUtil.insertList(list).setLog(log, "批量插入车场合同车辆配置").setNotPrint()
                            .executeBatch(false);
                            } catch (Exception e) {
                            log.debug("插入车场合同车辆配置报错，转为单条插入", e);
                            for (ContractParkbyvehicle t_contract_parkbyvehicle : t_contract_parkbyvehicles) {
                            try {
                            jdbcUtil.insert(t_contract_parkbyvehicle).execute();
                            } catch (Exception e1) {
                            log.error("单条插入车场合同车辆报错", e);
                            }
                            }
                            }
                            });
                            ...
                            */
                            ...
                            String sql = "select * from t_contract_parkbyvehicle where mainId=?  and deleted = 0";
                            List<ContractParkbyvehicle> t_contract_parkbyvehicleList = jdbcUtil.sql(sql).addIndexParam(id).query(ContractParkbyvehicle.class);
                                        ...
                                        List<Long> vehicleIdList = uservehicleauthority.getVehicleIdList();
                                            ...
                                            List<Long> vedepList = new ArrayList<>();
                                                ...
                                                List<Department> departmentList = getAuthorizedDepList(depList);
                                                    ...
                                                    for (ContractParkbyvehicle t_contract_parkbyvehicle : t_contract_parkbyvehicleList) {
                                                    Long depId = new Long(0);
                                                    if (realDataServiceRedis.getVehiclemap().get(ConverterUtils.toString(t_contract_parkbyvehicle.getVehicleid(), "")) != null) {
                                                    depId = realDataServiceRedis.getVehiclemap().get(ConverterUtils.toString(t_contract_parkbyvehicle.getVehicleid(), "")).getDepId();
                                                    }
                                                    if (depMap.containsKey(depId) || vehicleMap.containsKey(t_contract_parkbyvehicle.getVehicleid())) {
                                                    vehicleIds.add(t_contract_parkbyvehicle.getVehicleid());
                                                    }
                                                    ...
                                                    Map<Long, List<HashMap>> vehiclesbydep = new HashMap<>();
                                                        ...
                                                        List<String> dellist = new ArrayList<>();
                                                            ...
                                                            List<ConfigTreeNode> childnodes = v.getChildren();
                                                                ...
                                                                String delsql = "delete from t_contract_parkuserlim where mainid=? and userid = ?";
                                                                jdbcUtil.sql(delsql).addIndexParam(id, deleteuserid).executeUpdate();
                                                                }
                                                                }

                                                                if (adduserIdsArr != null && adduserIdsArr.length > 0) {
                                                                List<ContractParkuserlim> t_contract_parkuserlims = new ArrayList<>();
                                                                for (String userid : adduserIdsArr) {
                                                                ContractParkuserlim t_contract_parkuserlim = new ContractParkuserlim();
                                                                t_contract_parkuserlim.setMainid(id);
                                                                t_contract_parkuserlim.setUserid(Long.parseLong(userid));
                                                                t_contract_parkuserlims.add(t_contract_parkuserlim);
                                                                }
                                                                LoopUtil.splitLoop(t_contract_parkuserlims, 3000, list -> {
                                                                try {
                                                                ...
                                                                List<ConfigTreeNode> vehicleTreeNodes = vehicleTreeNode.getChildren();
                                                                    ...
                                                                    * 保存车场合同
                                                                    */
                                                                    public void save(ContractPark t_contract_park,Long userid) throws Exception {
                                                                    if (t_contract_park.getContractid() == 0) {//新增
                                                                    t_contract_park.setCreatedate(new Date());
                                                                    t_contract_park.setUserid(userid);
                                                                    jdbcUtil.insert(t_contract_park).insertColumn(ColumnSet.all()).execute();
                                                                    } else {//修改
                                                                    t_contract_park.setUpdatedate(new Date());
                                                                    jdbcUtil.update(t_contract_park).whereIdRefValueEQ().updateColumn(ColumnSet.all().minus(ContractPark.F_createdate,
                                                                    ContractPark.F_deleted,ContractPark.F_id,ContractPark.F_userid)).execute();
                                                                    }
                                                                    }
