                ...
                */
                public Map depNodeTree(Integer id, List<Department> depList) {
                    ...
                    List<Long> depids = new ArrayList<>();
                        String sql = "select * from ${tableschema}.${tablename}bydep where mainid = ?  and deleted = 0";
                        List<${entityName}bydep> datas = jdbcUtil.sql(sql).addIndexParam(id).query(${entityName}bydep.class);
                        Map<Long, Boolean> autoischeckMap = new HashMap<>();
                        if (ConverterUtils.isList(datas)) {
                        for (${entityName}bydep gpstransferbydep : datas) {
                        ...
                        List<String> dellist = new ArrayList<>();
                            ...
                            */
                            ...
                            String sql = "update ${tablename}bydep set deleted = 1 where mainid = ? and depid = ?";
                            jdbcUtil.sql(sql).addIndexParam(id, s).executeUpdate();
                            }
                            }
                            if (adddepIds != null) {
                            for (String depId : adddepIds) {
                            ${entityName}bydep ${tablename}bydep  = new ${entityName}bydep ();
                            ${tablename}bydep.setMainid(id);
                            ${tablename}bydep.setCreatedate(new Date());
                            ${tablename}bydep.setDeleted(new Long(0));
                            ${tablename}bydep.setId(new Long(0));
                            ${tablename}bydep.setDepid(ConverterUtils.toLong(depId));
                            jdbcUtil.insert( ${tablename}bydep).execute();
                            }
                            }
                            ...
                            */
                            ...
                            String sql = "update ${tableschema}.${tablename}byvehicle set deleted = 1 where mainid = ? and vehicleId = ?";
                            jdbcUtil.sql(sql).addIndexParam(id, deletevehicleId).executeUpdate();
                            }
                            }

                            List< ${entityName}byvehicle> ${tablename}byvehicles = new ArrayList<>();
                            if (addvehicleIds != null && addvehicleIds.length > 0) {//这边是添加的车辆批量添加
                            for (String vehicleId : addvehicleIds) {
                            ${entityName}byvehicle ${tablename}byvehicle=new ${entityName}byvehicle();
                            ${tablename}byvehicle.setMainid(id);
                            ${tablename}byvehicle.setCreatedate(new Date());
                            ${tablename}byvehicle.setDeleted(0L);
                            ${tablename}byvehicle.setVehicleid(ConverterUtils.toLong(vehicleId));
                            ${tablename}byvehicle.setId(0L);
                            ${tablename}byvehicles.add(${tablename}byvehicle);
                            }

                            LoopUtil.splitLoop(${tablename}byvehicles, 1000, list -> {
                            try {
                            jdbcUtil.insertList(list).setLog(log, "批量插入${tableconment}车辆配置").setNotPrint()
                            .executeBatch(false);
                            } catch (Exception e) {
                            log.debug("插入${tableconment}车辆配置报错，转为单条插入", e);
                            for (${entityName}byvehicle ${tablename}byvehicle : ${tablename}byvehicles) {
                            try {
                            jdbcUtil.insert(${tablename}byvehicle).execute();
                            } catch (Exception e1) {
                            ...
                            */
                            ...
                            String sql = "select * from ${tablename}byvehicle where mainId=?  and deleted = 0";
                            List<${entityName}byvehicle> ${tablename}byvehicleList = jdbcUtil.sql(sql).addIndexParam(id).query(${entityName}byvehicle.class);
                                        ...
                                        List<Long> vehicleIdList = uservehicleauthority.getVehicleIdList();
                                            ...
                                            List<Long> vedepList = new ArrayList<>();
                                                ...
                                                List<Department> departmentList = getAuthorizedDepList(depList);
                                                    ...
                                                    for (${entityName}byvehicle ${tablename}byvehicle : ${tablename}byvehicleList) {
                                                    Long depId = new Long(0);
                                                    if (realDataServiceRedis.getVehiclemap().get(ConverterUtils.toString(${tablename}byvehicle.getVehicleid(), "")) != null) {
                                                    depId = realDataServiceRedis.getVehiclemap().get(ConverterUtils.toString(${tablename}byvehicle.getVehicleid(), "")).getDepId();
                                                    }
                                                    if (depMap.containsKey(depId) || vehicleMap.containsKey(${tablename}byvehicle.getVehicleid())) {
                                                    vehicleIds.add(${tablename}byvehicle.getVehicleid());
                                                    }
                                                    ...
                                                    Map<Long, List<HashMap>> vehiclesbydep = new HashMap<>();
                                                        ...
                                                        List<String> dellist = new ArrayList<>();
                                                            ...
                                                            List<ConfigTreeNode> childnodes = v.getChildren();
                                                                ...
                                                                String delsql = "delete from ${tablename}userlim where mainid=? and userid = ?";
                                                                jdbcUtil.sql(delsql).addIndexParam(id, deleteuserid).executeUpdate();
                                                                }
                                                                }

                                                                if (adduserIdsArr != null && adduserIdsArr.length > 0) {
                                                                List<${entityName}userlim> ${tablename}userlims = new ArrayList<>();
                                                                for (String userid : adduserIdsArr) {
                                                                ${entityName}userlim ${tablename}userlim = new ${entityName}userlim();
                                                                ${tablename}userlim.setMainid(id);
                                                                ${tablename}userlim.setUserid(Long.parseLong(userid));
                                                                ${tablename}userlims.add(${tablename}userlim);
                                                                }
                                                                LoopUtil.splitLoop(${tablename}userlims, 3000, list -> {
                                                                try {
